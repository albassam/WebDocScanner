...
static int pdf_save_object_buffer(struct pdf_doc *pdf, struct dstr *buffer, int index) {
    ...
    if (object->type == OBJ_none)
        return -ENOENT;
	object->offset = dstr_len(buffer);
	dstr_printf(buffer, "%d 0 obj\r\n", index);
	switch (object->type) {
    case OBJ_stream:
    case OBJ_image: {
        dstr_append_data(buffer, dstr_data(&object->stream), dstr_len(&object->stream));
        break;
    }
    case OBJ_info: {
    ...
    case OBJ_page: {
        ...
        dstr_printf(buffer,
                ...
                "/Parent %d 0 R\r\n",
                pages->index);
        dstr_printf(buffer, "/MediaBox [0 0 %f %f]\r\n", object->page.width,
                object->page.height);
        dstr_append(buffer, "/Resources <<\r\n");
        dstr_append(buffer, "  /Font <<\r\n");
        for (struct pdf_object *font = pdf_find_first_object(pdf, OBJ_font);
             font; font = font->next)
            dstr_printf(buffer, "    /F%d %d 0 R\r\n", font->font.index, font->index);
        dstr_append(buffer, "  >>\r\n");
        // We trim transparency to just 4-bits
        dstr_append(buffer, "  /ExtGState <<\r\n");
        for (int i = 0; i < 16; i++) {
            dstr_printf(buffer, "  /GS%d <</ca %f>>\r\n", i, (float)(15 - i) / 15);
        }
        dstr_append(buffer, "  >>\r\n");

        if (image) {
            dstr_append(buffer, "  /XObject <<");
            for (; image; image = image->next)
                dstr_printf(buffer, "/Image%d %d 0 R ", image->index, image->index);
            dstr_append(buffer, ">>\r\n");
        }

        dstr_append(buffer, ">>\r\n");
        dstr_append(buffer, "/Contents [\r\n");
        for (int i = 0; i < flexarray_size(&object->page.children); i++) {
            struct pdf_object *child =
                flexarray_get(&object->page.children, i);
            dstr_printf(buffer, "%d 0 R\r\n", child->index);
        }
        dstr_append(buffer, "]\r\n");
        dstr_append(buffer, ">>\r\n");
        break;
    ...
    case OBJ_bookmark: {
        ...
        if (nchildren > 0) {
            ...
            dstr_printf(buffer, "/First %d 0 R\r\n", f->index);
            dstr_printf(buffer, "/Last %d 0 R\r\n", l->index);
        }
    ...
    case OBJ_pages: {
        ...
        for (struct pdf_object *page = pdf_find_first_object(pdf, OBJ_page);
             page; page = page->next) {
            npages++;
            dstr_printf(buffer, "%d 0 R ", page->index);
        }
        dstr_append(buffer,"]\r\n");
        dstr_printf(buffer, "/Count %d\r\n", npages);
        dstr_append(buffer, ">>\r\n");
        break;
    }
...
char* pdf_save_buffer(struct pdf_doc *pdf, size_t* length) {
	struct dstr *buffer = malloc(sizeof(struct dstr));
	buffer-> data = NULL;
    buffer->alloc_len = 0;
	buffer->used_len = 0;
    ...
    dstr_append(buffer, "%%PDF-1.2\r\n");
    /* Hibit bytes */
    dstr_printf(buffer, "%c%c%c%c%c\r\n", 0x25, 0xc7, 0xec, 0x8f, 0xa2);

    /* Dump all the objects & get their file offsets */
    for (int i = 0; i < flexarray_size(&pdf->objects); i++)
        if (pdf_save_object_buffer(pdf, buffer, i) >= 0)
            xref_count++;

    /* xref */
    xref_offset = dstr_len(buffer);
    dstr_append(buffer, "xref\r\n");
    dstr_printf(buffer, "0 %d\r\n", xref_count + 1);
    dstr_append(buffer, "0000000000 65535 f\r\n");
    for (int i = 0; i < flexarray_size(&pdf->objects); i++) {
        obj = pdf_get_object(pdf, i);
        if (obj->type != OBJ_none)
            dstr_printf(buffer, "%10.10d 00000 n\r\n", obj->offset);
    }
    ...
    dstr_printf(buffer, "/Root %d 0 R\r\n", obj->index);
    obj = pdf_find_first_object(pdf, OBJ_info);
    dstr_printf(buffer, "/Info %d 0 R\r\n", obj->index);
    ...
    dstr_printf(buffer, "/ID [<%16.16" PRIx64 "> <%16.16" PRIx64 ">]\r\n", id1, id2);
    dstr_append(buffer, ">>\r\n"
                "startxref\r\n");
    dstr_printf(buffer, "%d\r\n", xref_offset);
    dstr_append(buffer, "%%%%EOF\r\n");

    *length = dstr_len(buffer);
    return dstr_data(buffer);
...
size_t pdf_buffer_to_file(char* buffer, size_t length, const char* filename) {
	FILE *fp;
    size_t e = 0;
...
static pdf_object *pdf_add_raw_gray(struct pdf_doc *pdf, const uint8_t *data,
    ...
    dstr_printf(
		...
		len = dstr_len(&str) + (size_t)width * (size_t)height * 1 * 2 +
          strlen(endstream) + 1;
...
int pdf_add_raw_image(struct pdf_doc *pdf, struct pdf_object *page, float x,
                float y, float display_width, float display_height,
                const unsigned char* data, unsigned width, unsigned height) {
    struct pdf_object *obj = pdf_add_raw_gray(pdf, data, width, height);
